...
document.getElementById("mainArticle")[0].innerHTML = `<div id="feed" class="container-fluid"><div>`;
...
function fillFeed() {
    if (isBottomReached()) {
        ...
        let authorData = requestAuthorDetails(post.data[0].userId);
        let images = requestPostImages(ids.data[0].id);
    }
}
		...
		<article class="card w-75 mx-auto p-0">
			<div class="card-header">
				<h4 id="post-title" class="card-title">
					<a id="post-title" class="link-dark link-offset-2 link-offset-1-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover" href="">`+ + `</a>
				</h4>
